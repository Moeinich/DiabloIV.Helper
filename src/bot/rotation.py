...
HUMAN_REACTION_MIN = 0.01
HUMAN_REACTION_MAX = 0.04
HUMAN_KEY_HOLD_MIN = 0.02
HUMAN_KEY_HOLD_MAX = 0.06
HUMAN_POST_CAST_MIN = 0.02
HUMAN_POST_CAST_MAX = 0.06
HUMAN_HP_HESITATION_MIN = 0.02
HUMAN_HP_HESITATION_MAX = 0.06
HUMAN_PRE_KEY_MIN = 0.005
HUMAN_PRE_KEY_MAX = 0.02
HUMAN_RELEASE_GAP_MIN = 0.005
HUMAN_RELEASE_GAP_MAX = 0.015
HUMAN_SKILL_LOOP_MIN = 0.03
HUMAN_SKILL_LOOP_MAX = 0.08
HUMAN_ULT_LOOP_MIN = 0.05
HUMAN_ULT_LOOP_MAX = 0.12
DISTRACTED_PROBABILITY = 0.01
DISTRACTED_DELAY_MIN = 0.05
DISTRACTED_DELAY_MAX = 0.15
PRIORITY_NOISE_CHANCE = 0.1
...
_held_skill: Optional[str] = None
_held_hotkey: Optional[str] = None
...
def _record_skill_state(key: str, found: bool, enabled: bool, mode: str = ''):
    ...
    logging_helper.log_debug(f"[STATE] {key}: state={_skill_states[key]['state']}, found={found}, enabled={enabled}, mode={mode}")
...
def _cast_skill(c: bot_config.BotConfig, key: str, delay_mult: float) -> bool:
    ...
    if _held_skill and _held_skill != key:
        release_held_key()
...
def combat_rotation(c: bot_config.BotConfig) -> None:
    ...
    if _held_skill:
        ...
        still_valid = True
        if not c.is_skill_enabled(_held_skill):
            still_valid = False
        else:
            hp_ok = (c.skill_hp_min(_held_skill) <= hp_pct <= c.skill_hp_max(_held_skill))
            res_ok = (c.skill_resource_min(_held_skill) <= resource_pct <= c.skill_resource_max(_held_skill))
            if not hp_ok or not res_ok:
                still_valid = False
